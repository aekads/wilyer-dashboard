...
import {
  ...
  BookOpen, ChevronDown, Info, Layers, Folder, Search,
  Monitor, X, Settings, Calendar, Upload, Layout, Maximize2,
  AlignLeft, AlignRight, ChevronUp, ChevronDown as ChevronDownIcon,
  Maximize, ExternalLink, BookOpenCheck, Wand2
} from 'lucide-react'
...
function ZoneSettings({ layout, zones, selectedItemId, onRemoveItem, onDurationChange, activeZone }) {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      {/* Tab bar: Slides | Settings gear | Calendar | Delete red */}
      <div className="flex items-center gap-2 px-3 py-2.5 border-b border-gray-100">
        ...
        {/* Red trash icon on the far right */}
        <button
          className="w-8 h-8 flex items-center justify-center rounded-full bg-red-500 text-white hover:bg-red-600 transition-colors ml-auto"
      ...
      {tab === 'slides' && (
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {activeZoneItems.length === 0 ? (
            ...
            <div
              className="border-2 border-dashed border-gray-200 rounded-xl flex flex-col items-center justify-center py-12 text-center"
            >
...
function LayoutThumbCard({ layoutItem, zones, isSelected, onSelect, onDelete }) {
  ...
  return (
    <div
      ...
      style={{ width: 140 }}
    >
      {/* "New Layout" badge at top center */}
      ...
      <div className="mt-6 mx-2 rounded-lg overflow-hidden border border-gray-100" style={{ height: 80 }}>
        <div
          ...
          style={{
            display: 'grid',
            gridTemplateColumns: layoutItem.orientation === 'vertical' ? '1fr' : '1fr 1fr',
            gridTemplateRows: layoutItem.orientation === 'custom' ? '1fr 1fr' : '1fr',
            gap: '1px',
            background: '#e5e7eb'
          }}
        >
          {zones.map((z, idx) => {
            ...
            return (
              <div
                key={z.id}
                style={layoutItem.orientation === 'custom' && idx === 0 ? { gridColumn: '1 / -1' } : {}}
      ...
      <div className="px-2 pt-1.5 pb-2.5 text-center">
        <div className="text-[11px] font-semibold text-gray-700">{itemCount} Zones in {layoutItem.name}</div>
        <div className="text-[11px] text-gray-400">{totalDur.toFixed(1)}</div>
      </div>
...
function PreviewModal({ isOpen, onClose, layouts, zones, orientation }) {
...
const selectedOri = selectedLayout?.orientation || orientation
  ...
  const renderItemContent = (item) => {
    ...
    if (item.thumbnail_url) {
      return <img src={item.thumbnail_url} alt={item.media_name} className="w-full h-full object-cover" />
    }
    return (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 p-6 bg-gray-100 overflow-auto">
          <div
            className="mx-auto bg-white rounded-lg shadow-2xl overflow-hidden border-8 border-gray-800"
            style={{
              width: orientation === 'horizontal' ? 480 : 270,
              height: orientation === 'horizontal' ? 270 : 480,
            }}
          >
            <div className="w-full h-full" style={{ display: 'grid', gap: '2px', background: '#333', ...getGridStyle() }}>
              {zones.map((zone, idx) => {
                ...
                return (
                  <div
                    key={zone.id}
                    style={orientation === 'custom' && idx === 0 ? { gridColumn: '1 / -1' } : {}}
                    className="relative overflow-hidden bg-black"
                  >
                    {currentItem ? renderItemContent(currentItem) : (
                      <div className="w-full h-full flex items-center justify-center bg-gray-900">
                        <p className="text-white text-xs opacity-30">Empty</p>
                      </div>
                    )}
                  </div>
                )
...
export default function PlaylistBuilder() {
  ...
  const [name, setName] = useState('Hardik Prajapati')
  ...
  // activePanel: 'media' | 'widgets' | 'sequences'
  ...
  const currentZones = getZonesForOrientation(orientation)

  // Initialize empty layout for new playlist
  useEffect(() => {
    if (!id && !initialized) {
      const emptyLayout = makeEmptyLayout(orientation)
      setLayouts([emptyLayout])
      setSelectedLayoutId(emptyLayout.id)
      setActiveZone(currentZones[0]?.id || 'zone-main')
      setInitialized(true)
      setLoading(false)
    }
  }, [id, orientation, initialized])

  // Fetch playlist when editing
  useEffect(() => {
    if (id) fetchPlaylist()
  }, [id])

  // Update active zone when orientation changes
  useEffect(() => {
    if (layouts.length > 0 && currentZones.length > 0) {
    ...
    <Plus size={12} />New Layout
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <aside className="flex flex-col bg-white border-r border-gray-200 flex-shrink-0" style={{ width: 380 }}>
            ...
            {/* Tab row: Media | Widgets | Sequences */}
            ...
            {/* Media type filter + Design button */}
            {activePanel === 'media' && (
              ...
              {activePanel === 'media' && (
                loadingMedia ? (
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {mediaFiles.map(m => <DraggableMediaCard key={m.id} media={m} onAdd={addMediaToActive} />)}
                  </div>
                )
              )}
              ...
              {activePanel === 'widgets' && (
                loadingWidgets ? (
                  ...
                  <div className="space-y-2">
                    {widgetsList.map(w => <DraggableWidgetCard key={w.id || w.widget_type} widget={w} onAdd={addWidgetToActive} />)}
                  </div>
                )
              )}
          ...
          >
            <div className="flex-1 overflow-auto flex flex-col items-center pb-6">
              ...
              {/* Canvas top controls: Full Screen | Forward | Backward */}
              ...
              <div className="flex items-start gap-3 mt-6 px-4 flex-wrap justify-center">
                {layouts.map(layoutItem => (
                  <LayoutThumbCard
                    ...
                    onSelect={() => { setSelectedLayoutId(layoutItem.id); setActiveZone(currentZones[0].id) }}
